...
이 사본은 손으로 고치지 않는다 — `python tools/build_n8n_workflow.py` 가 원본과 함께
재생성하며, 테스트(`backend/tests/test_n8n_workflow.py`)가 커밋본과 생성 결과의 일치를
강제한다. 워크플로 수정은 생성기(또는 `n8n/sql/*.sql`)에서 한다.
