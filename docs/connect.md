...
- ✅ 통과 기준 (전부 화면):
  - 아는 테이블 미리보기 → **실제 값**이 보임 (전환 전엔 503 안내였다) + 필터 재조회 동작
...
잡 1회 최대 `AI_EMBED_JOB_CAP`(기본 1,000)개만 처리한다 — 2,342 테이블이면 **3회 실행**
(남은 개수가 화면에 표시된다). 서버 부하는 호출당 `AI_EMBED_BATCH`(32)·호출 간
`AI_EMBED_SLEEP_MS`(500ms)로 조절.
