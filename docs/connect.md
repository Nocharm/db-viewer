...
**미리보기는 목록에 등록한 테이블만 열린다** (기본 전부 차단). 관리 콘솔 →
*미리보기 허용 테이블*에서 수정 비밀번호(위 환경변수)를 입력하고, 검증에 쓸 테이블을 등록한다.
등록 전에는 버튼이 잠기고 API는 403이다 — 정상이다.

- ✅ 통과 기준 (전부 화면):
  - 허용 목록에 넣은 테이블 미리보기 → **실제 값**이 보임 (전환 전엔 503 안내였다) + 필터 재조회 동작
  - 목록에 없는 테이블은 미리보기 버튼이 잠김 (테이블 화면·ERD 우클릭 모두)
