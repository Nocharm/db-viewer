...
- 이 시점 미리보기는 **막혀 있는 게 정상** — `N8N_WEBHOOK_BASE`가 설정된 배포에서는
  합성 행 대신 "SOURCE_MODE=live로 전환하라"는 503을 낸다 (합성값은 실값과 구분되지 않아
  화면 검증을 오염시키므로 내주지 않는다). T2 검증은 "값 데이터 없음"이 정상.
...
서버 `.env`에서 `SOURCE_MODE=live`로 변경, 같은 파일에 `PREVIEW_ADMIN_PASSWORD`를 채운 뒤
→ `docker compose up -d backend` (재기동).

**미리보기는 목록에 등록한 스키마의 객체만 열린다** (기본 전부 차단). 관리 콘솔 →
*미리보기 허용 스키마*에서 수정 비밀번호(위 환경변수)를 입력하고, 검증에 쓸 스키마를 등록한다.
스키마 1건을 허용하면 그 안의 테이블·뷰가 전부 열리므로 최소 범위로 고른다.
등록 전에는 버튼이 잠기고 API는 403이다 — 정상이다.

- ✅ 통과 기준 (전부 화면):
  - 허용한 스키마의 테이블 미리보기 → **실제 값**이 보임 (전환 전엔 503 안내였다) + 필터 재조회 동작
  - 허용하지 않은 스키마의 테이블은 미리보기 버튼이 잠김 (테이블 화면·ERD 우클릭 모두)
  - ERD에서 확실한 FK 페어 하나 T2 검증 → containment **100%** 근처 + 1:N
    (조인 샘플은 **양쪽 테이블**이 허용 목록에 있어야 열린다)
  - 마스킹 대상 컬럼이 ●●● 로 보임 (지정했다면)
- 실패 시: 화면 오류 메시지가 원인을 말한다 — 403(허용 목록 미등록), 502(n8n 상태코드·본문 포함),
  빈 표 + "원본 소스가 0행"(W2는 실행됨). 증상별 조치표는 `n8n/workflows/README.md`.
...
- 응답의 `unjudged`가 반복해서 크면 LLM이 일부 페어 판정을 누락하고 있는 것 —
  같은 창이 재등장할 수 있다(모델·프롬프트 점검).
...
임베딩 서버는 **채팅 LLM과 별개 호스트**다 — `.env`에 아래를 채우고 backend 재기동 →
관리 콘솔 → **AI 임베딩 인덱싱** 실행.

```
EMBED_URL=http://<embed-host>:8000/v1   # /v1 루트·/embeddings 전체 경로 모두 허용
EMBED_MODEL=bge-m3
EMBED_TIMEOUT_SECONDS=30
```

변수명은 사내 다른 임베딩 사용 서비스와 동일하니 **그쪽 `.env` 값을 그대로 복사**하면 된다.
임베딩 서버는 무인증이라 토큰 설정이 없다 (`AI_API_KEY`는 채팅 전용 — 다른 호스트로
보내지 않는다). 채팅 LLM(`AI_BASE_URL`)이 없어도 의미 검색은 동작한다 — 이때는 LLM
재랭크 없이 코사인 순위를 그대로 쓴다.

잡 1회 최대 `EMBED_JOB_CAP`(기본 1,000)개만 처리한다 — 2,342 테이블이면 **3회 실행**
(남은 개수가 화면에 표시된다). 서버 부하는 호출당 `EMBED_BATCH`(32)·호출 간
`EMBED_SLEEP_MS`(500ms)로 조절.

- ✅ 확인: `?` 검색이 이름에 없는 의미 질의에도 결과를 낸다. 임베딩이 죽어도
  검색은 키워드 방식으로 자동 폴백된다(에러가 아님).
- 실패 시: `EMBED_URL` 오타·모델명 불일치 순으로 확인. 인덱싱 미설정은 400
  (`embed_url`·`embed_model`이 응답 context에 표시된다).
