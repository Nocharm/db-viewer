...
- **까그발 마커의 방향** — React Flow 엣지의 source→target 방향은 그래프 데이터에서
  ...
  엣지 방향이 뒤집힌 케이스를 단위 테스트에 넣는다. 이 정규화는 마커를 어느 쪽
  끝에 그릴지만 다룬다 — **카디널리티 값 자체**(`src:tgt` 표기의 `1:N`/`N:1`)는
  별개다. 애초엔 백엔드·프론트가 `src:tgt` 순서에 합의했으니 값을 그대로 믿어도
  되고 별도 정규화 레이어는 필요 없다고 가정했는데, 그 가정이 틀렸다 — 리뷰에서
  백엔드 계산식 자체가 뒤집혀 있었음이 드러났다(타깃이 유니크하면 `N:1`이어야
  하는데 `1:N`을 냈다). 변환 레이어를 얹는 대신 `backend/app/domain/validation.py`의
  계산식을 원천에서 고쳤다 — 값이 틀렸을 땐 정규화가 아니라 소스를 고친다
