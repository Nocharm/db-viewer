...
> **2026-09-11 변경.** 아래에서 기각했던 B안(공용 네트워크)을 **서비스별 선택지**로
> 되살렸다 — 외부에서 한 번 만드는 `dbv-shared`(`10.203.0.0/24`)에 backend가 상시
> 합류해 두면, 공유를 고른 서비스는 db-viewer compose 수정·재기동 없이 문만 내면 붙는다.
> 격리가 필요한 서비스는 여전히 B′. 별칭은 두 방식 모두 필수로 격상(공유 네트워크에서는
> 유일한 구별 수단). 소유는 어느 compose에도 두지 않는다 — db-viewer compose가 정의하면
> db-viewer `down` 시 다른 서비스 문이 달린 네트워크를 지우려다 실패하고, 서비스 compose가
> 참조하는 네트워크의 생명주기가 db-viewer에 종속된다. 운영 절차는 `docs/connect-sources.md` §1.
