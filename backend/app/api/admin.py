...
from app.services.pg_sources import ALLOWLIST_PREFIX
...
def list_preview_allowlist(db: Session = Depends(get_db)) -> dict:
    ...
    업무 Postgres의 허용 행(`pg:<slug>:<schema>`)은 여기서 빼고 연결 관리 화면이 보여준다
    — 카탈로그에 없는 스키마라 이 표에 섞이면 "객체 수"부터 채울 수 없다.
    """
    rows = [
        row for row in db.execute(
            select(PreviewAllowlist).order_by(PreviewAllowlist.schema)
        ).scalars().all()
        if not row.schema.startswith(ALLOWLIST_PREFIX)
    ]
    return {
...
def add_preview_allow(
    ...
    if schema.startswith(ALLOWLIST_PREFIX):
        raise HTTPException(400, {
            "message": "Postgres source schemas are unlocked on their own screen "
                       "(관리 콘솔 → 업무 Postgres 연결), not in this list",
