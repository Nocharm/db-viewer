...
- **Pretendard 단일 패밀리**(700 display·600 버튼·400 본문) — 한글·영문을 한 리듬으로 맞춘다.
  기준 시스템의 Inter를 대체: 사용자 화면이 한글 위주라 라틴 전용 폰트로는 한글이 OS 기본값
  (윈도우 맑은 고딕)으로 떨어져 조판이 무너졌다.
- 코드·식별자·SQL은 **JetBrains Mono**(0/O·1/l 판별), 그 안의 한글 값은 Pretendard로 대체
- 폰트는 npm 패키지로 **자가호스팅**(`pretendard`·`@fontsource/jetbrains-mono`) — 사내망은
  외부 CDN이 닿지 않는다. 스택 단일 소스는 `globals.css`의 `@theme`(Tailwind 유틸과 공유)
