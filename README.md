...
- **`/pg` — 업무 Postgres(선택)**: 서비스마다 따로 떠 있는 업무 DB를 그대로 조회한다. 연결은 **여러 개**
  등록할 수 있고 화면 상단에서 골라 오간다(등록은 관리 콘솔 — 아래 *업무 Postgres 연결*). 등록된 연결이
  하나라도 있어야 메뉴에 나온다. **카탈로그에 수집하지 않는 별도 소스**라 목록도 값도 그때그때 원본에서
  읽고, ERD·조인 검증·계보의 대상이 아니다. 값은 MSSQL과 같은 게이트를 거친다 — 스키마가 허용돼야 열리며
  (`pg:<연결>:<스키마>`), 조회는 감사 로그(`pg_preview`)에 남는다. 미리보기 표는 테이블 화면과 같은
  컴포넌트(필터·정렬·CSV·SQL 보기·분할)이고, SQL 보기만 Postgres 방언으로 나온다.
...
- **업무 Postgres 연결**: `/admin` → *업무 Postgres 연결*에서 연결을 등록·수정·삭제하고 스키마 단위로 값을
  연다(연결 테스트 버튼 포함). `.env`에는 **비밀번호 암호화 키 `PG_SOURCE_SECRET` 하나만** 둔다 — 대상이
  서비스마다 늘어나 목록을 배포로 관리할 수 없기 때문. 비밀번호는 이 키로 암호화해 서비스 DB에 저장되고
  어떤 응답에도 다시 실리지 않는다(키를 바꾸면 각 연결의 비밀번호를 다시 입력해야 한다). 편집 잠금은
  미리보기 허용 목록과 같다(시스템관리자 + `PREVIEW_ADMIN_PASSWORD`), 등록·수정·삭제·허용은 모두 감사 로그에 남는다.
  붙이려면 **두 가지가 따로** 필요하다 — ① **계정**: Postgres 인증은 DB마다 독립이라 대상 DB 담당자가 그쪽에
  읽기 전용 롤을 만들어 줘야 한다(`CREATE ROLE … LOGIN` + `GRANT CONNECT/USAGE/SELECT`). 우리 서비스 DB
  계정은 상대 DB에 통하지 않는다. ② **경로**: nginx는 HTTP 프록시라 이 경로와 무관하다(Postgres는 5432 TCP
  직접). 대상이 포트를 호스트에 열어 뒀으면 호스트 `172.48.0.1:<published>`(우리 네트워크 게이트웨이 =
  서버 자신)로, 안 열려 있으면 backend를 대상 스택의 네트워크에 `external`로 붙여 **컨테이너 이름**으로
  접속한다. 어느 쪽이든 등록 후 **연결 테스트**가 사유를 그대로 보여준다. 연결은 항상 read-only로 열고
...
| `/pg` 메뉴가 안 보임 | 등록된 연결 0건이거나 `PG_SOURCE_SECRET` 미설정 (`/api/pg/status`의 `enabled`·`secret_configured`로 구분) |
| `/pg`에서 「값 잠김」 | 그 연결의 해당 스키마가 안 열려 있다 — `/admin` → 업무 Postgres 연결 → *스키마* → 「값 허용」 |
| 연결 테스트 실패 | 사유가 그대로 표시된다 — 호스트/포트(컨테이너 네트워크), 계정·비밀번호, 대상 `pg_hba.conf` 순으로 확인 |
| 연결 등록·사용이 503 | `PG_SOURCE_SECRET` 미설정, 또는 키를 바꿔 저장된 비밀번호를 못 연다(각 연결의 비밀번호 재입력) |
