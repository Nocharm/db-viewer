...
- **`/` — 테이블 브라우저**: 좌측 목록에서 객체를 고르면 우측 상세(컬럼·사용 뷰·유사 테이블·관계)가 뜬다.
  좌측 배치는 조인키 바의 토글로 고른다 — **3열**(스키마 레일 + 목록, 기본) 또는 **트리**. 트리 배치는 좌측 열을
  아예 두지 않고 **상세 패널의 객체 이름 자리가 곧 선택기**다: 누르면 스키마 > 객체 트리가 내려오고, 필터 버튼은
  플라이아웃으로 3열과 같은 축(종류·카테고리·스키마)을 열며 「필터 전부 해제」가 있다. 배치는 브라우저별로 유지된다.
  상세의 [미리보기]·[쿼리 보기] 옆 **[다이어그램]**이 계보 맵을 아코디언으로 펼친다 — 아래 참조.
  읽는 뷰가 없는 테이블은 그릴 계보가 없어 버튼이 잠기고, 열어 둔 상태로 그런 테이블에 오면 섹션이 접힌다
  (다시 계보가 있는 객체로 가면 저절로 펼쳐진다).
  상세의 컬럼 칩을 클릭하면 바로 떠나지 않고, 그 컬럼을 왼쪽 키로 놓고 `/verify`로 갈지 묻는 확인 카드가
  클릭 지점 옆(화면 가장자리에서는 반대쪽으로 뒤집힘)에 뜬다. MSSQL이 아닌 소스에서는 잠금 안내만 보인다.
- **계보 다이어그램** (테이블 상세 안, 탭 4종): 뷰는 **SQL 연동**(좌측 1/3에 정의 SQL;
  맵 노드 호버 → 해당 줄이 화면 중앙보다 살짝 위로 스크롤, SQL 줄 호버 → 해당 노드로 줌. 양쪽 다 버퍼를 둬
  마우스가 스치는 동안은 반응하지 않는다. 강조 근거는 *이름 일치*라 동명 컬럼은 함께 물든다) →
  **소스 흐름**(FROM·JOIN 대상과 조인 조건) →
  **컬럼 계보**(출력 컬럼이 어느 base 컬럼에서 왔는지, 가로 위치 = depth),
  테이블·뷰 공통으로 **영향 범위**(이 객체를 읽는 뷰를 depth 순 팬아웃 — 컬럼을 바꾸거나 지울 때 번지는 범위,
  간선 라벨이 그 뷰가 실제로 읽는 컬럼이다). 탭 우측 ⓘ에 네 탭의 설명과 조작법이 모여 있다.
  모든 맵은 휠 줌 + 캔버스 드래그 팬. 노드 카드에는 *그 맥락에서 쓰이는 컬럼*만 먼저 뜨고, 카드 안 「미사용 N」
  행을 누르면 나머지까지 펼쳐지며 같은 레인의 아래 노드가 자동으로 밀려난다. 카드를 클릭하면 정보 카드가
  **클릭 지점 옆**(우하단, 화면 가장자리에서는 반대쪽으로 뒤집힘)에 열린다 — 행·컬럼 수, AI 요약(없으면 생성,
  있으면 다시 생성 버튼), 「이 노드 중심으로 보기」와 「상세 열기」가 한 줄. 지금 보고 있는 객체는 「현재 맵」으로 표시된다.
  간선 라벨은 붐비지 않는 쪽 끝(소스 흐름은 소스 옆, 영향 범위는 읽는 뷰 앞)에
  앉고, 누르면 그 관계의 상세(조인 조건·읽는 컬럼 전체)가 모달로 열린다. 영향 범위는 그 탭을 처음 열 때 조회하며,
  받는 동안은 고스트 그래프(카드·점선·시머)가 자리를 지킨다.
  API: `GET /api/lineage/views/{id}` · `GET /api/lineage/objects/{id}/impact`
  (파싱 결과는 이미 `view_deps`·`view_joins`·`view_lineage_flat`에 있어 새 파싱 없이 조립만 한다).
- **`/verify` — 조인 검증**: 좌/우 테이블을 고르면 컬럼 페어 후보(점수순 자동 + 수동 지정)가 뜬다. 값 조회 없는
  ...
  **④ 확정** 순으로 진행한다. 검증 대기 목록(AI 제안 버튼 포함)이 진입점.
  최상단 「지금 확인 중인 연결」 상자에서 컬럼을 바로 갈아끼울 수 있고, 「값 미리보기」는 두 테이블을
  화면 아래 미리보기 섹션에 분할로 열어 지금 페어의 컬럼을 강조한다(미리보기 허용 스키마만).
- **`/erd` — 읽기 전용 ERD**: 확정된 관계 + FK 전체 그래프를 연결요소(컴포넌트) 단위로 배치해 한 번에 보여준다.
  간선은 ELK가 계산한 **직교 경로**를 그대로 그려 간선마다 자기 회랑을 가진다(같은 두 레이어 사이 간선이 한
  세로선에 겹치지 않는다). 테이블 검색과 호버 컬럼 내비게이션(라벨은 노드에 안 겹치는 구간에 뜬다)을 제공하고,
  노드는 헤더를 잡아 옮길 수 있다(세션 한정 — 옮긴 노드의 간선만 꺾은선으로 폴백, 좌하단 컨트롤의 초기화 버튼이
  자동 배치로 되돌림). 1-hop 확장·조인 빌더는 없다(뷰 계보는 테이블 상세의 lineage 패널이 담당). `/erd?focus=<object_id>`로
...
- **`/trace` — 값 추적**: 화면에서 본 값 하나로 어느 `스키마.객체.컬럼`에 저장돼 있는지 찾는다. 소스와 스키마(=시스템)를
  ...
  표기 변형(기본)·정확·부분(후보 `VALUE_PROBE_CONTAINS_MAX_TABLES`개 이하만). 「다른 스키마의 연관 뷰 포함」을
  켜면 선택 스키마의 테이블을 읽는 다른 스키마의 뷰까지 후보에 넣고, 그중 정책으로 제외된 뷰는 진행 카드의 ⓘ에
  사유와 함께 나열한다. 실행 중에는 찾기 버튼이 진척(스피너·바)을 보이고 호버하면 「중단」이 된다. 결과 행의 「미리보기 열기」가
  `컬럼 = 실제 저장값` 필터로 미리보기를 연다. 히트가 뷰면 「쿼리 보기」로 그 뷰의 정의 SQL을 행 아래에 펼쳐
  히트 컬럼을 강조해 보여준다(테이블 상세의 뷰에도 같은 버튼이 있다).
  미리보기 허용 스키마만 검색되고 감사 로그(`value_probe`)에 남는다.
...
- Docker 네트워크: `172.48.0.0/16` (사내 대역 충돌 회피 요청값 — RFC1918 사설 대역 아님에 유의).
  다른 서비스 DB를 붙이는 네트워크는 서비스마다 둘 중 하나 — 공유 `dbv-shared` `10.203.0.0/24`(기본 — 서버에 생성 완료, `docker-compose.yml`의 backend가 external로 상시 합류) 또는 전용 `dbv-<서비스키>` `10.203.<n>.0/24`(`n`은 1부터, 첫 연결이 1 사용 — `docs/connect-sources.md` §1)
...
- **감사 로그**: `/admin` → *감사 로그* 탭(`/admin?tab=audit`, 관리자 전용 — 예전 `/admin/audit`는 이 탭으로 리다이렉트).
  실값 반출(테이블 미리보기·조인 샘플·값 추적), 권한·설정 변경(허용 목록·화이트리스트·감춤 표시·관계 확정·카테고리),
  소스·수집 조작, 로그인·접근 거부가 한 표에 최신순으로 쌓인다 — 기간 칩·요약 타일, 동작/요청자/대상 드롭다운, 행 클릭 상세 모달.
  요청자는 항상 인증 사용자(요청 본문의 이름은 무시)
