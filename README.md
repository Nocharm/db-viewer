...
- **계보 다이어그램** (테이블 상세 안, 탭 4종): 뷰는 **SQL 연동**(좌측 1/3에 정의 SQL;
  ...
  **노드 오른쪽 바깥**(마우스 높이)에 열린다 — 행·컬럼 수, AI 요약(없으면 생성, 있으면 다시 생성 버튼),
  「이 노드 중심으로 보기」와 「상세 열기」가 한 줄. 지금 보고 있는 객체는 「현재 맵」으로 표시된다.
  간선 라벨은 길면 줄바꿈되고, 누르면 그 관계의 상세(조인 조건·읽는 컬럼 전체)가 모달로 열린다.
  영향 범위는 그 탭을 처음 열 때 조회한다.
