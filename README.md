...
- **`/` — 테이블 브라우저**: 좌측 목록에서 객체를 고르면 우측 상세(컬럼·사용 뷰·유사 테이블·관계)가 뜬다.
  좌측 배치는 조인키 바의 토글로 고른다 — **3열**(스키마 레일 + 목록, 기본) 또는 **트리**(스키마 > 객체를
  한 열로 합침, 좁은 폭에선 선택된 행만 플로팅으로 남고 눌러서 드롭다운·필터 모달). 배치는 브라우저별로 유지된다.
  상세의 [미리보기]·[쿼리 보기] 옆 **[다이어그램]**이 계보 맵을 아코디언으로 펼친다 — 아래 참조.
- **계보 다이어그램** (테이블 상세 안, 탭 4종): 뷰는 **소스 흐름**(FROM·JOIN 대상과 조인 조건) →
  **컬럼 계보**(출력 컬럼이 어느 base 컬럼에서 왔는지, 가로 위치 = depth) → **SQL 연동**(좌측 1/3에 정의 SQL;
  ...
  모든 맵은 휠 줌 + 캔버스 드래그 팬. 노드 카드에는 *그 맥락에서 쓰이는 컬럼*만 먼저 뜨고, 카드를 클릭하면
  미사용 컬럼까지 펼쳐지며 같은 레인의 아래 노드가 자동으로 밀려난다. 노드 호버 시 팝오버(행·컬럼 수, AI 요약
  또는 생성 버튼, 「이 노드 중심으로 보기」·「상세 열기」)가 뜨고 핀으로 고정된다.
