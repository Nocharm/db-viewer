...
- **`/pg` — 업무 Postgres(선택)**: 앱 서버 옆에 떠 있는 별도 업무 DB를 그대로 조회한다. `PG_SOURCE_DSN`
  (읽기 전용 계정)이 설정된 배포에서만 메뉴에 나온다. **카탈로그에 수집하지 않는 별도 소스**라 목록도 값도
  그때그때 원본에서 읽고, ERD·조인 검증·계보의 대상이 아니다. 값은 MSSQL과 같은 게이트를 거친다 —
  관리 콘솔의 미리보기 허용 목록에 `pg:<스키마>`가 있어야 열리며, 조회는 감사 로그(`pg_preview`)에 남는다.
  미리보기 표는 테이블 화면과 같은 컴포넌트(필터·정렬·CSV·SQL 보기·분할)이고, SQL 보기만 Postgres 방언으로 나온다.
...
| `/pg` 메뉴가 안 보임 | `PG_SOURCE_DSN` 미설정 — `.env` 채우고 backend 재기동 (`/api/pg/status`의 `enabled`로 확인) |
| `/pg`에서 「값 잠김」 | 미리보기 허용 목록에 `pg:<스키마>`가 없다 — 관리 콘솔 목록의 `pg:`로 시작하는 줄에서 허용 |
| `/pg` 502 | 메시지에 psycopg 오류가 그대로 실린다 — DSN 호스트/포트·컨테이너 네트워크·계정 권한 순으로 확인 |
