...
- **계보 다이어그램** (테이블 상세 안, 탭 4종): 뷰는 **SQL 연동**(좌측 1/3에 정의 SQL;
  ...
  **클릭 지점 옆**(우하단, 화면 가장자리에서는 반대쪽으로 뒤집힘)에 열린다 — 행·컬럼 수, AI 요약(없으면 생성,
  ...
  카드의 **컬럼 행을 클릭하면** 그 컬럼을 왼쪽 키로 놓고 `/verify`로 갈지 묻는 확인 카드가 같은 규칙으로 뜬다
  (MSSQL이 아닌 소스에서는 잠금 안내). 간선 라벨은 붐비지 않는 쪽 끝(소스 흐름은 소스 옆, 영향 범위는 읽는 뷰 앞)에
...
- **`/erd` — 읽기 전용 ERD**: 확정된 관계 + FK 전체 그래프를 연결요소(컴포넌트) 단위로 배치해 한 번에 보여준다.
  ...
  자동 배치로 되돌림). 펼친 노드의 컬럼 행을 클릭하면 그 컬럼으로 `/verify`에 갈지 묻는다. 1-hop 확장·조인 빌더는 없다(뷰 계보는 테이블 상세의 lineage 패널이 담당). `/erd?focus=<object_id>`로
