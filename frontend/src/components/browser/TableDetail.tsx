...
interface Props {
   ...
   * / opens the verify page; when target is given (a join-check result row) the verify page
   * prefills both source and target. */
  onOpenColumn: (
    columnId: number,
  ...
  return (
    <li className="flex items-center gap-3 text-sm">
      ...
      <button
        ...
        onClick={() => onOpenColumn(item.src_column_id, {
...
export function TableDetail({
  detail, loading, previewLoading, previewAllowed, onPreview, onOpenErd, canJumpToPreview,
  onJumpToPreview, onSelectTable, onOpenColumn,
}: Props) {
  const { t } = useI18n();
